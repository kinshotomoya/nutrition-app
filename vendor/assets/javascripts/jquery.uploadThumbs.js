			...
			if (file && (file.type && file.type.match(/^image/)		// image ?
				...
				reader.onload = function (file, i) { return function () {
					...
					if (pathName.match(/recipe/)){
						var tag = '<img src="'+ this.result +'" alt="'+ file.name +'" title="'+ file.name +' ('+ Math.round( file.size / 1024 ) +'kb)' +'" class="thumb2" />';
					}else{
						var tag = '<img src="'+ this.result +'" alt="'+ file.name +'" title="'+ file.name +' ('+ Math.round( file.size / 1024 ) +'kb)' +'" class="thumb1" />';
					}
	...
	// legacy IE
		...
		if (file && !file.match('fakepath') && file.match(/\.(jp[eg]+|png|gif|bmp)$/i)) {
			...
			img.onload = function () {
				...
				var tag = '<img src="'+ this.src +'" alt="'+ filename +'" title="'+ this.src +'" class="legacy thumb1 thumb2" />';
...
$.fn.uploadThumbs.clear = function (option) {
	...
	var $images = (typeof option.position == 'number')
				?  this.siblings('img.thumb1 thumb2')
				:  $( option.position ).find('img.thumb1 thumb2');
